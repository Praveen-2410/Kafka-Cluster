...
check_data_dir() {
  ...
  ssh "$USER@$ip" "test -d ~/$DATA_DIR && [ -n \"\$(ls -A ~/$DATA_DIR 2>/dev/null)\" ]"
}

### Get cluster ID from meta.properties ###
get_cluster_id() {
  local ip=$1
  ssh "$USER@$ip" "grep cluster.id ~/$DATA_DIR/meta.properties | cut -d'=' -f2"
}
...
backup_data_dir() {
  ...
  ssh "$USER@$ip" "mkdir -p ~/$BACKUP_DIR && cp -r ~/$DATA_DIR/* ~/$BACKUP_DIR/"
}
...
format_broker() {
  ...
  ssh "$USER@$ip" "sudo docker run --rm -v ~/$CONFIG_PATH:$KAFKA_CONFIG_DOCKER_PATH -v ~/$DATA_DIR:/var/lib/kafka/data $DOCKER_IMAGE $FORMAT_CMD -t $CLUSTER_ID -c $KAFKA_CONFIG_DOCKER_PATH"
}
